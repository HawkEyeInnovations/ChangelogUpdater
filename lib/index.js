...
const closestPkgFile = findupSync('package.json');
...
function newChangelog() {
	...
	const closestPkgJson = JSON.parse(
		fs.readFileSync(closestPkgFile, { encoding: 'utf8' })
	);

	version = closestPkgJson.version;
	repository = closestPkgJson.repository;
